...
test_list = get_split_list("F:\\zIMseg\\pointcloud\\partanno_train_test_val\\train_test_split\\shuffled_test_file_list.json")
val_list = get_split_list("F:\\zIMseg\\pointcloud\\partanno_train_test_val\\train_test_split\\shuffled_val_file_list.json")
...
for kkk in range(len(class_name_list)):
	...
	class_test_list = [str for str in test_list if class_name in str]
	class_val_list = [str for str in val_list if class_name in str]
	class_train_len = len(class_train_list)
	class_test_len = len(class_test_list)
	class_val_len = len(class_val_list)
	class_len_all = class_train_len+class_test_len+class_val_len
	exceed_32 = 0
	exceed_64 = 0
	...
	for idx in range(name_num):
		...
		if (proper_name in class_train_list or proper_name in class_test_list or proper_name in class_val_list):
	...
	for idx in range(name_num):
		...
		#get voxel models
		proper_name = "shape_data/"+class_name+"/"+name_list[idx]
		if not(proper_name in class_train_list or proper_name in class_test_list or proper_name in class_val_list):
			continue
	...
	fstatistics.write("train set: "+str(class_train_len)+"\n")
	fstatistics.write("test set: "+str(class_test_len)+"\n")
	fstatistics.write("val set: "+str(class_val_len)+"\n")
	fstatistics.write("total: "+str(class_len_all)+"\n")
	fstatistics.write("non-repeat total: "+str(class_len_all_real)+"\n")
